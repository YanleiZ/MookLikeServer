...
<div id="formwrapper">
	<h3>查看课程</h3>
	<form action="CourseServlet?method=update&pageNo=${page.pageNo }&pageSize=${page.pageSize }" method="post">
	...
	<fieldset>
		...
		<div>
			...
			<input type="text" name="name" id="name" value="${course.name }" size="60" maxlength="200" /> 
			<br />	
		</div>
		<div>
			<label for="descp">备注</label>
			<input type="text" name="descp" id="descp" value="${course.descp }" size="60" maxlength="200" /> 
			<br />	
		</div>
		<div>
			<label for="img">添加图片</label>
			<input type="button" onclick="image('img')" id="attachs" value="添加">
			<input type="hidden" name="img" id="img" value="${course.img }" size="60" maxlength="200" /> 
			<img id="img_image" src="upload_image/${course.img }">
			<br />	
		</div>
		<div class="enter">
		    <input name="return" type="button" class="buttom" value="返回列表页面" onclick="window.location = 'CourseServlet'"/>
		</div>
	</fieldset>
	</form>
</div>

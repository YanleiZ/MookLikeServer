...
<div id="formwrapper">
	<h3>编辑首页</h3>
	<form action="IndexServlet?method=update&pageNo=${page.pageNo }&pageSize=${page.pageSize }" method="post">
...
<div>
			...
			<select name="course" id="courseIds" >
				...
				<option value="${s.id }"  <c:if test="${s.id eq index.course.id }">selected="selected"</c:if>
				>${s.name}</option>
				</c:forEach>
			</select> 
		</div>
<div>
			...
			<select name="ppts" id="pptsIds" >
				...
				<option value="${s.id }"  <c:if test="${s.id eq index.ppts.id }">selected="selected"</c:if>
				>${s.name}</option>
				</c:forEach>
			</select> 
		</div>
<div>
			...
			<select name="videos" id="videosIds" >
				...
				<option value="${s.id }"  <c:if test="${s.id eq index.videos.id }">selected="selected"</c:if>
				>${s.name}</option>
				</c:forEach>
			</select> 
		</div>
<div>
			...
			<select name="words" id="wordsIds" >
				...
				<option value="${s.id }"  <c:if test="${s.id eq index.words.id }">selected="selected"</c:if>
				>${s.name}</option>
				</c:forEach>
			</select> 
		</div>
<div>
			...
			<select name="tests" id="testsIds" >
				...
				<option value="${s.id }"  <c:if test="${s.id eq index.tests.id }">selected="selected"</c:if>
				>${s.name}</option>
				</c:forEach>
			</select> 
		</div>

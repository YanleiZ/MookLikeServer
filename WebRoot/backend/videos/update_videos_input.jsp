...
<div id="formwrapper">
	<h3>编辑教学视频</h3>
	<form action="VideosServlet?method=update&pageNo=${page.pageNo }&pageSize=${page.pageSize }" method="post">
	<input type="hidden" name="id" value="${videos.id }">

	<fieldset>
		<legend>教学视频信息</legend>
		<div>
			<label for="name">名称</label>
			<input type="text" name="name" id="name" value="${videos.name }" size="60" maxlength="200" /> 
			<br />	
		</div>
		<div>
			<label for="settime">上传时间</label>
			<input type="text" name="settime" id="settime" value="${videos.settime }" size="8" maxlength="200" /> 
			<br />	
		</div>
<div>
			...
			<select name="course" id="courseIds" >
				...
				<option value="${s.id }"  <c:if test="${s.id eq videos.course.id }">selected="selected"</c:if>
				>${s.name}</option>
				</c:forEach>
			</select> 
		</div>
		<div>
			...
			<input type="text" name="descp" id="descp" value="${videos.descp }" size="60" maxlength="200" /> 
			<br />	
		</div>
		<div>
			<label for="img">上传文件</label>
			<input type="button" onclick="image('docts')" id="attachs" value="点此上传">
			<input type="hidden" name="docts" id="docts" value="${videos.docts }" size="60" maxlength="200" /> 
			<a id="docts_file" href="upload_file/${videos.docts }">下载文件</a>
			<br />	
		</div>
		<div>
			<label for="content">简介</label>
			<div style="margin:0px 0px 0px 130px;">
			<textarea rows="10" cols="100" name="content" id="content" >${videos.content }</textarea>
			</div>
			<br />	
		</div>
		<div>
			<label for="attachs">添加简介附件</label>
		...
		<div>
			...
			<input type="hidden" name="img" id="img" value="${videos.img }" size="60" maxlength="200" /> 
			<img id="img_image" src="upload_image/${videos.img }">
			<br />	
		</div>
		<div class="enter">
		    <input name="submit" type="submit" class="buttom" value="提交" />
		    <input name="reset" type="reset" class="buttom" value="重置" />
		    <input name="return" type="button" class="buttom" value="返回列表页面" onclick="window.location = 'VideosServlet'"/>
		</div>
	</fieldset>
	</form>
</div>

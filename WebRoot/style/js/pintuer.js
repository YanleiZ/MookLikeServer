...
function login(win){
    ...
    data:{
    	method:"loginf",
     	username:$(win).parent().parent().find('#username').val(),
     	password:$(win).parent().parent().find('#password').val()
    },
    success: function(data){
    		if(data=="true") {
    			var oldloginsuccess = $("#loginsuccess .dialog").children();
    			$(win).parent().parent().html($("#loginsuccess .dialog").children());
    			$(".dialog-close,.close").each(function(){
    				$(this).click(function(){
    					$("#loginsuccess .dialog").html(oldloginsuccess);
    					location.reload();
    				});
    			});
    		}
    		else {
    			$(win).parent().parent().find('.login-help').css("display","block");
    			$(win).parent().parent().find('.login-help li').html(data);
    		}
    }
 });
}
...
$(function(){
	$("#answerbut").click(function(){
		var str="";
		for(var i=1;i<=$("#qescount").val();i++){
			str+=$("#ans"+$("input[name='qes"+i+"']:checked").val()).val();
		}
		$(".answer ").html(str);
	});
